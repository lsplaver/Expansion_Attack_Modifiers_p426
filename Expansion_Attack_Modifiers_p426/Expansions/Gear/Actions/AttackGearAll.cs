    ...
    public class AttackGearAll : IActionGear
    {
        ...
        public void Actions(Battle battle, CharacterAttackModifierGearInventoryHitChance currentCharacter, CharacterAttackModifierGearInventoryHitChance targetCharacter, ActionTypes characterAction, string strExpansions)
        {
            ...
            //        {
                        Random random = new Random();
                        double randomHitChance = random.NextDouble();
                        if (randomHitChance <= currentCharacter.CharacterInventory.WeaponHitChances[0].HitChance)
                        {
                            if (j >= 0)
                            {
                                attackName = currentCharacter.AvailableActionHitChances[j].Name;
        ...
        private void DisplayResultsAttackModified(Character currentCharacter, string attackName, Character targetCharacter, int adjustedDamageDealt, List<AttackModifierOffensive> attackModifierOffensives, List<AttackModifierDefensive> attackModifierDefensives)
        {
            Console.WriteLine($"It is {currentCharacter.Name}'s turn...");
            Console.WriteLine($"{currentCharacter.Name} used {attackName} on {targetCharacter.Name}.");
